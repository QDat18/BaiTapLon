...
function loadNews() {
    fetch('./data/news.json')
        ...
        .then(data => {
            ...
            if (data.length === 0) {
                newsContainer.innerHTML = '<p class="text-center text-muted">Không có tin tức nào vào lúc này.</p>';
                return;
            }
            
            // Add swiper structure
            newsContainer.classList.add('swiper', 'news-swiper');
            ...
            newsContainer.appendChild(prevBtn);
            newsContainer.appendChild(nextBtn);
            newsContainer.appendChild(pagination);
            
            data.forEach(news => {
                const newsCard = document.createElement('div');
                newsCard.className = 'swiper-slide';
                    ...
                    <a href="news_detail.html?id=${news.id}" class="text-decoration-none">
                        <div class="card h-100">
                            ...
                            <div class="card-body">
                                ...
                                <p class="text-muted small">${news.startDate}</p>
                            </div>
                        </div>
                    </a>
                `;
                
                swiperWrapper.appendChild(newsCard);
            });
        })
        .catch(error => {
            console.error('Lỗi khi tải tin tức:', error);
            document.getElementById('news-container').innerHTML = 
                '<p class="text-danger text-center">Không thể tải dữ liệu tin tức.</p>';
        });
}